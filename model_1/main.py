...
def train(train_lr_imgs, train_hr_imgs):
    ...
    t_target_image = tf.placeholder(dtype='float32', shape=(batch_size, 512, 512, 1), name='t_target_image')
    ...
    net_vgg, vgg_target_emb = Vgg19_simple_api(input = (t_target_image_224 + 1) / 2, reuse=False)
    _, vgg_predict_emb = Vgg19_simple_api(input = (t_predict_image_224 + 1) / 2, reuse=True)
    ...
    for epoch in range(0, n_epoch_init):
        ...
        for idx in range(0, len(train_hr_imgs), batch_size):
            ...
            b_imgs_hr = np.asarray(b_imgs_hr).reshape((batch_size,512,512,1))
...
if __name__ == '__main__':
    ...
    n_patients_train = len(data) - config.n_eval # leave 5 patients for hand evaluation
    train_lr_imgs = []
    train_hr_imgs = []
    counter = 0
    for patient, values in data.items():
